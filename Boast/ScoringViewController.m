...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, TIME_LIMIT*NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        [self endTurn];
    });
...
- (void)endTurn {
    ...
    if (self.score >= gameData.currentBid) {
        ...
        [self sendToScreen:@"question-success"];
        // wait 5s, and send to end game if needed
        // scoreboard - at end of game
        // after clicking go button on next round, go to pick_category
    } else {
        gameData.highBidder.skipRound = YES;
        [self sendToScreen:@"question-failure"];
    }
